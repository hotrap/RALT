...
class SSTIterator {
 public:
  SSTIterator() {}
  SSTIterator(const ImmutableFile<KeyCompT>* file);
  SSTIterator(const ImmutableFile<KeyCompT>* file, SKey key);
 ...
 public:
  class ImmutableFileAsyncSeekHandle {
    public:
      ...
      template<typename T>
      std::optional<SSTIterator<KeyCompT>> next(T&& aio_info) {
        if (state_ == 0) {
          ...
          if (id.value().first == -1) {
            return SSTIterator<KeyCompT>(file_.data_block_.get_enum_iterator_to_first());
          }
        ...
        if (state_ == 1) {
          ...
          return SSTIterator<KeyCompT>(file_.data_block_.get_enum_iterator(id.value().first, id.value().second));
        }
        return {};
      }
    private:
      ...
      std::unique_ptr<typename FileBlock<IndexKey, KeyCompT>::AsyncSeekHandle<typename ImmutableFile<KeyCompT>::ImmutableFileAsyncSeekHandle::IndexBS>> index_async_seek_handle_;
  ...
  class TempFileOpen {
    public:
      TempFileOpen(const ImmutableFile<KeyCompT>& file) : file_(file) { fd_ = file_.file_ptr_->get_fd(); }
  ...
  typename FileBlock<DataKey, KeyCompT>::EnumIterator estimate_seek(SKey key) {
    ...
    auto id = index_block_.upper_offset(key, tmp_open.get_fd());
    if (id == -1) return {};
  ...
  typename FileBlock<DataKey, KeyCompT>::EnumIterator seek(SKey key) const {
    if (comp_(range_.second.ref(), key) < 0) return {};
    TempFileOpen tmp_open(*this);
    auto id = index_block_.get_block_id_from_index(key, tmp_open.get_fd());
  ...
  std::pair<int, int> rank_pair(const std::pair<SKey, SKey>& range, const std::pair<bool, bool> exclude_info) const {
    ...
    if (comp_(range_.second.ref(), L) < 0)
      ...
      auto id = index_block_.get_block_id_from_index(L, tmp_open.get_fd());
      if (id == -1) {
        retl = 0;
      } else {
    ...
    if (comp_(range_.second.ref(), R) <= 0)
      ...
      auto id = index_block_.get_block_id_from_index(R, tmp_open.get_fd());
      if (id == -1) {
        retr = 0;
      } else {
...
template<typename KeyCompT>
SSTIterator<KeyCompT>::SSTIterator(const ImmutableFile<KeyCompT>* file) : file_block_iter_(file->data_block().get_enum_iterator_to_first()) {}


template<typename KeyCompT>
SSTIterator<KeyCompT>::SSTIterator(const ImmutableFile<KeyCompT>* file, SKey key) : file_block_iter_(file->seek(key)) {}
