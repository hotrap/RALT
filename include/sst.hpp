...
class SSTIterator {
 public:
  ...
  SSTIterator() {}
  SSTIterator(const ImmutableFile<KeyCompT, ValueT>* file);
  SSTIterator(const ImmutableFile<KeyCompT, ValueT>* file, SKey key);
...
class ImmutableFile {
 ...
 public:
  ...
  class TempFileOpen {
    public:
      TempFileOpen(const ImmutableFile<KeyCompT, ValueT>& file) : file_(file) { fd_ = file_.file_ptr_->get_fd(); }
      ~TempFileOpen() { file_.file_ptr_->release_fd(fd_); }
      int get_fd() const { return fd_; } 
      
    private:
      const ImmutableFile<KeyCompT, ValueT>& file_;
      int fd_;
  };
...
template<typename KeyCompT, typename ValueT>
SSTIterator<KeyCompT, ValueT>::SSTIterator(const ImmutableFile<KeyCompT, ValueT>* file) : file_block_iter_(file->data_block().get_enum_iterator_to_first()) {}


template<typename KeyCompT, typename ValueT>
SSTIterator<KeyCompT, ValueT>::SSTIterator(const ImmutableFile<KeyCompT, ValueT>* file, SKey key) : file_block_iter_(file->seek(key)) {}
